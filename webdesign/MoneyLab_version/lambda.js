...
//////////////////////////////////////////////////////////////////////////////////
	...
	$toggle_menu.click(function() {
		...
		$('.pushmenu-push').one('webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend', function() {alignSidenotes();});
		$('#menu_bar').fadeToggle();
	});
...
////SIDENOTES BECOME TIP BOXES IF SCREEN IS SMALL, AND THEY ARE CENTERED
	function sideToBox() {
		if ($(window).width() <= "500") {	//SUBSTITUTE '400' WITH THE DESIRED MINIMUM SIZE
			// $('#button_to_close_menu_on_small_devices').show();					// UNCOMMENT THESE FOUR LINES AND 
			// $('#button_to_close_menu_on_small_devices').click(function() {		// PUT HERE THE NAME OF THE BUTTON
			// 	$menuLeft.toggleClass('pushmenu-open');								// THAT IS SUPPOSED TO CLOSE THE MENU
			// })																	// WHEN THE SCRREN IS SMALL

			$('#chapter8').hide();
			$('.in-text').click(function() {
				$('.tip').css({"display": "block", "max-width": "90%"}).offset({top: top-30, "left": 5});
			});
			$('.footnoteRef').click(function(ev) {
				ev.stopPropagation();
				ev.preventDefault();
...
////CHANGE FONT SIZE AND ALIGN SIDENOTES
